...
module.exports = class Upload {
	...
	async uploadChunk() {
		...
		let fileId = null;
		try {
			[fileId] = await Promise.all([uploadPromise, streamPromise]);
			log('Chunk completed');
		} catch (err) {
			...
			if (streamErrored) {
				log('Waiting for chunk upload to finish');
			...
			try {
				// TODO Implement retry - at present whole upload fails if this errors
				fileId = await this.put(null, 0, '*');
			} catch (posErr) {
				log('Did not succeed in checking continuation point');
				throw posErr;
			}
		}
	 ...
	 * @returns {Promise} - Resolves to file ID if upload complete, or `null` if not.
	...
	async put(stream, size, range) {
		...
		if ([200, 201].includes(statusCode)) {
			...
			try {
				...
				throw new Error('Put could not parse file ID on completion');
			}
		}
		...
		if (statusCode === 308) {
			...
			if (!resRange) {
				...
				try {
					...
					throw new Error(`Put could not parse range header: ${resRange}`);
				}
			}
		...
		throw new Error(`Put invalid status code ${statusCode}`);
	}
};
